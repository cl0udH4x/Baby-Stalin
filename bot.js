...
bot.on('message', message => {
    ...
    // Ping
		if (msg === prefix + 'PING') { // This checks if msg (the message but in all caps), is the same as the prefix + the command in all caps.
				...
				message.channel.send('There is no pong in Mother Russia!'); // This 'sends' the message to the channel the message was in. You can change what is in the message to whatever you want.
		...
		if (msg.startsWith(prefix + 'PURGE')) { // This time we have to use startsWith, since we will be adding a number to the end of the command.
				...
				async function purge() {
					...
					if(!message.member.roles.find("name", "Admin++")){
						message.channel.send("You are not part of the central committee");
						return;
					}
						...
						if (isNaN(args[0])) {
								...
								message.channel.send('You are enemy of Soviet People! You will be executed! Use number twat! \n Usage: ' + prefix + 'purge <amount>'); //\n means new line.
    ...
    if (msg.startsWith(prefix + 'HELP')) { // We're also going to use a seperate JSON file, so we need to call it.
        ...
        if (msg === `${prefix}HELP`) { // If they only type this, lets ONLY show the commands for regular users
            ...
            for (var cmd in commands) { // We should start creating the commands json first.
                ...
                if (commands[cmd].group.toUpperCase() === 'MEMBER OF THE GLORIOUS COMMUNIST PARTY') {
		...
		if(msg === prefix + "NICOISGAY"){
				for(var i = 0; i < 3;i++){
					message.delete();
					message.channel.send("@schokonico#0048 ur mom gay");
					message.channel.send(" @schokonico#0048 ur dad lesbian");
					message.channel.send("@schokonico#0048 ur sister mister");
					message.channel.send("@schokonico#0048 ur granny tranny");
					message.channel.send("@schokonico#0048 ur grandpap a trap");
					message.channel.send("@schokonico#0048 family tree LGBT");
					message.channel.send("@schokonico#0048 ur super mega colossal tripple decker gay");
				}
		}
		if(msg === prefix + "MAN"){
      message.delete();
      message.channel.send("https://www.marxists.org/archive/marx/works/download/pdf/Manifesto.pdf")
    }
		if(msg.startsWith("NOHOMO")){
			message.delete();
			message.channel.send("Homosexuality is not allowed in Soviet Russia! Off to the gulag with you!")
		}
  if(msg === prefix + "DICE"){
      var rawGuess =  Math.floor(Math.random()*6)+1;
      const embed = new Discord.RichEmbed()
          .setColor(0x1D82B6)
          message.channel.send({embed: {
              color: 0x1D82B6,
...
bot.login('NDI4NjUyNTc4Nzg4ODY4MTE2.DZ2QSQ.IweYYUrNs0rKHTlZvEUuvO6Nxvs');
